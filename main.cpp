...
/**
 * This is a quick-n-dirty console application to parse the contents
...
struct Chunk
{
	...
	unsigned int getSize()
	{
		...
		{
			totalSize += static_cast<unsigned int>(rawData.size());
		}
...
void analyseLwo(const std::string& filename, bool modify)
{
	...
	Chunk form;
	form.id = "FORM";
	form.size = static_cast<unsigned int>(vectorBuffer.size());
...
int main(int argc, char* argv[])
{
	if (argc == 1)
	{
		std::cout << "Usage: LwoChunkAnalyzer <pathToLwoFile>";
		return 0;
	}
